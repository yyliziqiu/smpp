...
var (
	...
	dlrDateLayout1 = "0601021504"
	dlrDateLayout2 = "060102150405"
)
...
func (r *Dlr) String() string {
	return fmt.Sprintf(dlrFormat, r.Id, r.Sub, r.Dlvrd, r.submitDate(), r.doneDate(), r.Stat, r.Err, r.Text)
}

func (r *Dlr) submitDate() string {
	return r.SDate.Format(dlrDateLayout1)
}

func (r *Dlr) doneDate() string {
	return r.DDate.Format(dlrDateLayout1)
}

func DlrParse(s string) (*Dlr, error) {
	...
	date1, err := dlrParseDate(match[4])
	if err != nil {
		return nil, ErrInvalidDlrFormat
	}
	date2, err := dlrParseDate(match[5])
	if err != nil {
		return nil, ErrInvalidDlrFormat
	}
	text := ""
...
func dlrParseDate(s string) (time.Time, error) {
	date, err := time.Parse(dlrDateLayout1, s)
	if err != nil {
		date, err = time.Parse(dlrDateLayout2, s)
		if err != nil {
